...
- Keep temporary coordination details out of the PR description. This includes merge or apply order, prerequisites, merge restrictions, and draft or readiness instructions. If the user asks to record these on GitHub, put them in a PR comment.
- Report test results, verification steps, deployment status, and completion summaries in the chat response, not in the PR description or comments, unless the user explicitly asks to post them on GitHub.
- Never include a `Validation` section in the PR description. Do not move that material into a comment as a workaround.
- Creating or updating a PR does not require a follow-up comment. Do not post PR comments unless the user asks.
